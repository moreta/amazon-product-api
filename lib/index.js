...
var runQuery = function (credentials, method, options) {

  var xml2jsOptions = options.xml2jsOptions || {}
    ...
    if (typeof cb === 'function') {
      request(url, function (err, response, body) {
        ...
        if (err) {
          ...
          parseXML(body, xml2jsOptions, function (err, resp) {
            if (err) {
              cb(err);
            } else {
...
var createClient = function (credentials, options) {
  return {
    itemSearch: runQuery(credentials, 'ItemSearch', options),
    itemLookup: runQuery(credentials, 'ItemLookup', options),
    browseNodeLookup: runQuery(credentials, 'BrowseNodeLookup', options)
  };
};
